...
class LocalGistAiModelTest {
  ...
  fun test_analyzeOffline_detectsComposeWithoutRemember() {
    ...
    val hasRememberWarning =
      result.optimizationSuggestions.any {
  ...
  fun test_analyzeOffline_detectsHardcodedSecrets() {
    ...
    val hasSecretWarning =
      result.optimizationSuggestions.any { it.contains("Security Warn") && it.contains("secrets") }
  ...
  fun test_analyzeOffline_detectsLogAndPrintStatements() {
    ...
    val hasPrintWarning = result.optimizationSuggestions.any { it.contains("debug print/log") }
  ...
  fun test_analyzeOffline_detectsFilesWithoutExtension() {
    ...
    val files = listOf("Dockerfile" to "FROM openjdk:17", "helper_script" to "echo hello")

    val result = LocalGistAiModel.analyzeOffline(description, files)

    val hasExtensionWarning =
      result.optimizationSuggestions.any {
  ...
  fun test_analyzeGist_gracefullyFallsBack_onGeminiFailure() = runTest {
    ...
    val result =
      LocalGistAiModel.analyzeGist(description, files, "AIzaSy_fake_invalid_key_for_testing")
