...
class GistOfflineE2ETest {
    ...
    fun setUp() {
        ...
        // Initialize in-memory database with synchronous direct executors for deterministic test assertions
    ...
    @Test
    fun test_offline_create_read_update_delete_and_sync_transitions() = runTest(testDispatcher) {
        // ==========================================
        ...
        assertEquals("Should contain exactly 1 local draft when created offline", 1, offlineGists.size)
        ...
        assertFalse("isDirty must be false for freshly created local drafts", createdDraft.gist.isDirty)
        ...
        // Assert that the draft itself was modified, but sync flags are unchanged (isLocalOnly=true, isDirty=false)
        ...
        assertEquals("Description should reflect the update", updatedDraftDesc, draftAfterUpdate.gist.description)
        ...
        val syncedGistEntity = GistEntity(
            id = syncedGistId,
        ...
        val syncedFileEntity = GistFileEntity(
            fileId = "file_id_999",
            gistId = syncedGistId,
            filename = "app.kt",
            type = "text/plain",
        ...
        assertEquals("Description must show modifications", modifiedRemoteDesc, modifiedRemoteGist?.gist?.description)
        assertFalse("Must NOT be localOnly as it originally exists on GitHub", modifiedRemoteGist?.gist?.isLocalOnly ?: true)
        assertTrue("Must be flagged as isDirty because it has offline unsynced modifications", modifiedRemoteGist?.gist?.isDirty ?: false)
        ...
        assertNull("Deleted draft must be completely purged from local storage", listAfterDraftDeletion.find { it.gist.id == draftId })
        ...
        // It should gracefully delete locally immediately, in preparation for eventual remote reconciliation.
        ...
        val syncDraftId = repository.createLocalDraft(syncDraftDesc, listOf(syncDraftFile to syncDraftContent), isPublic = true, isPinned = false)

        val syncRemoteId = "remote_sync_id_xyz"
        val syncRemoteEntity = GistEntity(
            id = syncRemoteId,
        ...
        val syncRemoteFile = GistFileEntity(
            fileId = "file_xyz",
            gistId = syncRemoteId,
            filename = "main.kt",
            type = "text/plain",
        ...
        assertEquals("Must have exactly 3 unsynced items before sync (1 deletion, 1 draft, 1 dirty)", 3, unsyncedBefore.size)
        ...
        assertFalse("Synced draft must NOT have a temp draft ID anymore", newSyncedId.startsWith("draft_"))
        assertFalse("Synced draft must have isLocalOnly reset to false", syncedDraft?.gist?.isLocalOnly ?: true)
        assertFalse("Synced draft must have isDirty reset to false", syncedDraft?.gist?.isDirty ?: true)
        assertEquals("Filename should be preserved", syncDraftFile, syncedDraft?.files?.firstOrNull()?.filename)
        assertEquals("Content should be preserved", syncDraftContent, syncedDraft?.files?.firstOrNull()?.content)
        ...
        assertFalse("Synced dirty Gist must have isLocalOnly = false", syncedDirtyGist?.gist?.isLocalOnly ?: true)
        assertFalse("Synced dirty Gist must have isDirty reset to false", syncedDirtyGist?.gist?.isDirty ?: true)
        assertEquals("Description should reflect updated content", syncRemoteModifiedDesc, syncedDirtyGist?.gist?.description)
        assertEquals("Content should reflect updated content", syncRemoteModifiedContent, syncedDirtyGist?.files?.firstOrNull()?.content)
        ...
        assertTrue("All local unsynced states must be empty after full synchronization", unsyncedAfter.isEmpty())
    }
    ...
    private class OfflineSimulatingGitHubApiService : GitHubApiService {
        ...
        val defaultOwner = GistOwnerResponse(
            login = "testUser",
            id = 12345,
        ...
        override suspend fun getGists(): List<GistResponse> {
            checkOffline()
            return gistsList
        }
