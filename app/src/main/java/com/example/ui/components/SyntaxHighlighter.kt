...
object SyntaxHighlighter {
    ...
    private fun getStyleForType(type: TokenType): SpanStyle {
        return when (type) {
            TokenType.COMMENT -> SpanStyle(color = Color(0xFF6A9955), fontStyle = FontStyle.Italic) // Muted Green
            ...
            TokenType.KEYWORD -> SpanStyle(color = Color(0xFF569CD6), fontWeight = FontWeight.Bold) // Light Blue
    ...
    private val pythonPatterns = listOf(
        ...
        TokenType.ANNOTATION to "(?:@\\w+)",
        TokenType.KEYWORD to "\\b(?:False|None|True|and|as|assert|async|await|break|class|continue|def|del|elif|else|except|finally|for|from|global|if|import|in|is|lambda|nonlocal|not|or|pass|raise|return|try|while|with|yield)\\b",
        TokenType.TYPE to "\\b[A-Z]\\w*\\b",
    ...
    private val xmlHtmlPatterns = listOf(
        TokenType.COMMENT to "(?:<!--[\\s\\S]*?-->)",
    ...
    fun highlight(text: String, filename: String): AnnotatedString {
        ...
        val patterns = when (extension) {
            "kt", "kts", "java" -> kotlinJavaPatterns
            "js", "jsx", "ts", "tsx", "mjs", "cjs" -> jsTsPatterns
            "py" -> pythonPatterns
            "json" -> jsonPatterns
            "xml", "html", "svg", "xhtml" -> xmlHtmlPatterns
            "sql" -> sqlPatterns
            "css" -> cssPatterns
