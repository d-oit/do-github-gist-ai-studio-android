...
object LocalGistAiModel {
    ...
    /**
     * Pure offline code-parsing heuristic engine (Simulated Local LLM Model).
     * High-fidelity, extremely fast on-device analysis.
     */
    fun analyzeOffline(description: String, files: List<Pair<String, String>>): GistAiAnalysis {
        ...
        val summaryText = if (description.isNotBlank()) {
        ...
        val branchingTokens = listOf(
            "if ", "if(", "for ", "for(", "while ", "while(", "when", "&&", "||",
            "catch", "filter", "map {", "forEach"
        )
        ...
        files.forEach { (name, content) ->
            ...
            when {
                ...
                lowerName.endsWith(".js") || lowerName.endsWith(".ts") || lowerName.endsWith(".jsx") || lowerName.endsWith(".tsx") || lowerName.endsWith(".mjs") -> tags.add("#JavaScript")
                ...
                lowerName.endsWith(".yml") || lowerName.endsWith(".yaml") || lowerName.endsWith(".toml") -> tags.add("#YAML-TOML")
                lowerName.endsWith(".c") || lowerName.endsWith(".cpp") || lowerName.endsWith(".h") || lowerName.endsWith(".hpp") -> tags.add("#C-CPP")
                ...
                lowerName.endsWith(".sh") || lowerName.endsWith(".bash") || lowerName.endsWith(".zsh") || lowerName == "makefile" || lowerName == "dockerfile" -> tags.add("#Shell")
            }

            // Keyword tag parsing
            if (content.contains("retrofit") || content.contains("ktor") || content.contains("OkHttpClient")) {
                tags.add("#Networking")
            }
            if (content.contains("Room") || content.contains("Dao") || content.contains("Entity") || content.contains("sqlite")) {
                tags.add("#Database")
            }
            ...
            if (content.contains("println(") || content.contains("System.out.print") || content.contains("Log.d(") || content.contains("Log.e(")) {
                hasPrintStatements = true
            }

            // Performance/maintainability suggestions
            if (content.lines().size > 150) {
                optims.add("File '$name' is quite long (${content.lines().size} lines). Consider splitting it into smaller, decoupled helper modules.")
            }
        }
        ...
        val commentLines = files.sumOf { (_, content) ->
            content.lines().count { it.trim().startsWith("//") || it.trim().startsWith("#") || it.trim().startsWith("/*") || it.trim().startsWith("*") }
        }
        ...
        if (hasHardcodedSecrets) {
            optims.add("Security Warn: Hardcoded API tokens or credential markers detected! Avoid embedding secrets directly in source files.")
        }
        if (hasComposeWithoutRemember) {
            optims.add("Compose Warn: Detected state creation (mutableStateOf) without 'remember'. State will be lost on recomposition.")
        }
        if (hasPrintStatements) {
            optims.add("Clean Code: Remove debug print/log statements (e.g., println) before checking in code.")
        }
        if (files.any { it.first.isBlank() }) {
            optims.add("Filenames: Ensure all files have a valid name. While GitHub Gist supports files without extensions, specifying an extension (e.g. '.kt', '.py', '.json') enables rich syntax highlighting.")
        } else {
            val nonExtensionSpecialNames = setOf("makefile", "dockerfile", "license", "jenkinsfile", "vagrantfile", "readme")
            val hasFileWithoutExtension = files.any { (name, _) ->
                !name.contains(".") && !nonExtensionSpecialNames.contains(name.lowercase())
            }
            if (hasFileWithoutExtension) {
                optims.add("Syntax Highlighting: Some files do not have an extension. GitHub Gist supports any name, but adding a standard file extension ensures proper color formatting and syntax rendering.")
            }
        }
        if (optims.isEmpty()) {
            optims.add("Code looks extremely pristine and follows modern offline development architecture.")
        }
    ...
    /**
     * Calls Gemini 3.5-Flash model via REST API for deep generative analysis.
     * Strictly conforms to gemini-api skill instructions.
     */
    private suspend fun analyzeWithGemini(
        ...
        val prompt = buildString {
            append("You are a expert code analyzer assistant. Analyze the following Gist draft and return a structured JSON response matching the GistAiAnalysis schema.\n\n")
            ...
            append("recommendedTags (list of 3-5 standard hashtags starting with #, e.g., #Kotlin, #Compose), ")
            append("and generate a 2-sentence summary.\n\n")
            append("Your response MUST be a single raw JSON object with these EXACT keys: ")
            append("\"summary\", \"complexityScore\", \"complexityLevel\", \"maintainabilityIndex\", \"optimizationSuggestions\", \"recommendedTags\". ")
        ...
        okHttpClient.newCall(request).execute().use { response ->
            ...
            val suggestions = responseMap["optimizationSuggestions"] as? List<String> ?: listOf("No suggestions offered.")
            @Suppress("UNCHECKED_CAST")
    ...
    private fun parseGeminiResponse(json: String): Map<String, Any> {
        ...
        val outerMap = baseAdapter.fromJson(json) as? Map<String, Any> ?: throw Exception("Invalid JSON structure")
        val candidates = outerMap["candidates"] as? List<Map<String, Any>> ?: throw Exception("Missing candidates")
        val content = candidates.firstOrNull()?.get("content") as? Map<String, Any> ?: throw Exception("Missing content")
        ...
        return baseAdapter.fromJson(text) as? Map<String, Any> ?: throw Exception("Failed to parse inner response JSON")
    }
}
