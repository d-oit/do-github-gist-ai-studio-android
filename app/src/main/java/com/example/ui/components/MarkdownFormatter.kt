    ...
    val (newText, newSelection) = when (formatType) {
        ...
        "code" -> {
            if (selection.collapsed) {
                ...
                updated to TextRange(selection.start + offset, selection.start + offset + selectedText.length)
            }
        }
        "link" -> {
            if (selection.collapsed) {
                ...
                val updated = before + insert + after
                updated to TextRange(selection.start + selectedText.length + 3, selection.start + selectedText.length + 14)
            }
        }
        "num_list" -> {
            if (selection.collapsed) {
